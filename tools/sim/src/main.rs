//! The app, in a window, driven by your keyboard.
//!
//! ```
//! cargo run -p sim
//! ```
...
use symbian_ui::{Fonts, Handled, Key, KeyEvent, Modifiers, Palette, Softkey, Theme};
...
fn load(name: &str) -> Vec<u8> {
    ...
    std::fs::read(&p).unwrap_or_else(|e| {
        panic!("{p}: {e}\nRun tools/mkfonts.sh first, and run this from the SDK root.")
    })
}
...
fn main() {
    let (d11, d11b, d9) = (load("ui11"), load("ui11b"), load("ui9"));
    let f11 = BitmapFont::new(&d11).unwrap();
    let f11b = BitmapFont::new(&d11b).unwrap();
    let f9 = BitmapFont::new(&d9).unwrap();
